...
def _gaussian_sample(grid: Grid, sigma: float = 3.0) -> Optional[Point]:
    """Sample using Gaussian strategy to find narrow passages"""
    grid_size = len(grid)
    
    # Sample two random points
    p1 = (randint(0, grid_size - 1), randint(0, grid_size - 1))
    p2 = (randint(0, grid_size - 1), randint(0, grid_size - 1))
    
    # Check if one is in collision and one is free
    p1_free = grid[p1[1]][p1[0]] == 0
    p2_free = grid[p2[1]][p2[0]] == 0
    
    if p1_free == p2_free:
        # Both free or both collision, use regular sampling
        return None
    
    # One free, one collision - sample near boundary
    if p1_free:
        free_point = p1
    else:
        free_point = p2
    
    # Sample with Gaussian distribution around free point
    x = int(free_point[0] + gauss(0, sigma))
    y = int(free_point[1] + gauss(0, sigma))
...
def run(
    ...
    *,
    ...
    gaussian_ratio: float = 0.5,
    sigma: float = 3.0,
    animate: bool = False,
    visualize: Visualizer = None,
) -> tuple[List[Point], int]:
    """Gaussian PRM: samples near obstacles to find narrow passages"""
    ...
    # Sample points using Gaussian strategy
    attempts = 0
    gaussian_samples = int(num_samples * gaussian_ratio)
    regular_samples = num_samples - gaussian_samples
    
    # Gaussian samples
    while len(nodes) < gaussian_samples + 2 and attempts < gaussian_samples * 3:
        attempts += 1
        sample = _gaussian_sample(grid, sigma)
    ...
    # Regular uniform samples
    attempts = 0
    while len(nodes) < num_samples + 2 and attempts < regular_samples * 3:
        attempts += 1

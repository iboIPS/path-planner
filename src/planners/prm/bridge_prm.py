...
def _bridge_test_sample(grid: Grid, max_dist: float = 5.0) -> Optional[Point]:
    """Bridge test: sample midpoint between two obstacle points"""
    grid_size = len(grid)
    
    # Sample first point in obstacle
    attempts = 0
    p1 = None
    while attempts < 20:
        attempts += 1
    ...
    # Sample second point nearby, also in obstacle
    attempts = 0
    p2 = None
    while attempts < 20:
        attempts += 1
        dx = randint(-int(max_dist), int(max_dist))
        dy = randint(-int(max_dist), int(max_dist))
        x = p1[0] + dx
        y = p1[1] + dy
    ...
    # Check if midpoint is collision-free (in narrow passage)
    if 0 <= mid_x < grid_size and 0 <= mid_y < grid_size:
        if grid[mid_y][mid_x] == 0:
            return midpoint
...
def run(
    ...
    *,
    ...
    bridge_max_dist: float = 5.0,
    animate: bool = False,
    visualize: Visualizer = None,
) -> tuple[List[Point], int]:
    """Bridge Test PRM: focuses sampling on narrow passages"""
    ...
    # Sample points using bridge test
    attempts = 0
    bridge_samples = int(num_samples * bridge_ratio)
    regular_samples = num_samples - bridge_samples
    
    # Bridge test samples (for narrow passages)
    while len(nodes) < bridge_samples + 2 and attempts < bridge_samples * 5:
        attempts += 1
        sample = _bridge_test_sample(grid, bridge_max_dist)
